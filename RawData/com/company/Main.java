...
public class Main {
    ...
    public static void main(String[] args) {
        ...
        for (Car car : cars) {
            if (car.getCargo().getCargoType().equals(command) && command.equals("fragile")) {
                for (Tire tireSet : car.getTires()) {
                    if (tireSet.getPressure() < 1) {
                        output.add(car);
                        break;
                    }
                }
            }else if (car.getCargo().getCargoType().equals(command) && command.equals("flammable")){
